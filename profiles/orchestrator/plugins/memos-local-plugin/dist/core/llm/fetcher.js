    ...
    while (attempt <= opts.maxRetries) {
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                throw new MemosError(errCodeForStatus(resp.status), `HTTP ${resp.status} from ${opts.provider}`, { provider: opts.provider, url: opts.url, status: resp.status, body: text });
            }
