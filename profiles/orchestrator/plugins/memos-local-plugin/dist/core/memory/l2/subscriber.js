...
function settleInjectedPolicies(input) {
    ...
    if (episode.outcome === "success") {
        ...
        for (const policyId of injectedIds) {
            if (!viewedSet.has(policyId))
                continue;
            const row = input.repos.policies.getById(policyId);
            if (!row)
                continue;
            viewedRows.push(row);
    ...
    if (episode.outcome === "failure") {
        for (const policyId of injectedIds) {
            ...
            if (row.status === "active" && next >= 2)
                nextStatus = "candidate";
            else if (row.status === "candidate" && next >= 3)
                nextStatus = "archived";
            input.repos.policies.upsert({
                ...row,
                status: nextStatus,
...
function readInjectedPolicyIds(episodeId, repos) {
    const rows = repos.episodePolicyInjections.listPolicyIdsForEpisode(episodeId);
    return rows.filter((id) => typeof id === "string" && id.trim().length > 0);
}
function extractViewedPolicyIds(traces) {
    const out = new Set();
    for (const trace of traces) {
        for (const toolCall of trace.toolCalls ?? []) {
            if ((toolCall.name ?? "").toLowerCase() !== "memos_get")
                continue;
            if (toolCall.errorCode && toolCall.errorCode.trim().length > 0)
                continue;
...
function degradeFailStreakOf(meta) {
    const n = meta?.degradeFailStreak;
    return typeof n === "number" && Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0;
}
function withDegradeMeta(meta, streak) {
    return {
        ...(meta ?? {}),
...
function refreshPolicyGainOnSuccess(input) {
    const mergeFamilies = new Set();
    for (const row of input.viewedRows) {
        if (row?.mergeFamily)
            mergeFamilies.add(row.mergeFamily);
    }
    if (mergeFamilies.size === 0)
        return;
    const candidates = input.repos.policies
        .list({ status: "candidate", limit: 5_000 })
        .filter((row) => (row.mergeFamily
...
function isInfraFailure(traces) {
