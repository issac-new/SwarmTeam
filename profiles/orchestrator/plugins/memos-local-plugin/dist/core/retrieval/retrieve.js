...
import { effectiveReadOnlyInjectionProfile, effectiveSkillInjectionMode, isIrDomain, } from "../domain.js";
import { ids } from "../id.js";
import { rootLogger } from "../logger/index.js";
import { collectDecisionGuidance } from "./decision-guidance.js";
import { buildQueryWithExtract, extractRepairTaskSection, isSoftwareRepairPrompt, rawQueryText, } from "./query-builder.js";
...
import { mergeRetrievePlanOverride, resolveInjectionProfilePlan, } from "./injection-profile.js";
const log = rootLogger.child({ channel: "core.retrieval" });
const RETRIEVAL_QUERY_EXTRACT_TIMEOUT_MS = 5_000;
export function taskProtocolOnlyPacket(ctx, now) {
    ...
    const { packet } = toPacket({
        ...
        episodeId: ctx.episodeId ?? `adhoc-episode-${ids.span()}`,
        taskProtocol,
    });
    return packet;
}
...
function withInjectionProfileOpts(deps, opts) {
    if (!isIrDomain(deps.config.domain))
        return opts;
    const profilePlan = resolveInjectionProfilePlan(effectiveReadOnlyInjectionProfile(deps.config));
...
async function runAll(deps, ctx, opts, plan) {
    ...
    try {
        ...
        const traceOnly = !experienceOnly &&
        ...
        const tier2Promise = wantTier2 && !experienceOnly && !noUsableChannel
            ? runTier2({ repos: deps.repos, config: deps.config, now: deps.now }, {
                ...
                excludeSessionId: sessionId && !deps.config.lightweightMemory
                    ? sessionId
                    : undefined,
            })
            : Promise.resolve({ traces: [], episodes: [] });
        const tier2ExperiencePromise = wantTier2 && (!traceOnly || experienceOnly) && !noUsableChannel
        ...
        const queryText = compiled.text || (ctx.userText ?? "");
...
function renderTaskProtocol(ctx) {
    if (ctx.reason !== "turn_start")
        return null;
    const userText = ctx.userText;
    if (!isSoftwareRepairPrompt(userText))
        return null;
    ...
    protocol.push("", "### Patch-first completion contract", "1. The goal is a small non-empty source `git diff`, not an explanation, test-only change, or proof that the bug already appears fixed.", "2. Use read-only commands to locate the target, but do not exceed eight inspect/search commands before the first source edit. If evidence points to one function/class, write the minimal tentative patch with the exact-replacement script, then test and inspect `git diff`.", "3. If a visible bug or hint closure appears above, treat it as the current task action queue: grep one exact visible identifier, inspect the containing source once or twice, then edit the candidate source behavior before searching tests broadly.", "4. Closure-stop rule: after you have inspected the candidate function/class named by a closure, the next tool call must create an outer-write `/tmp/memos_edit.py` exact-replacement script for the smallest source patch. Do not inspect tests first.", "5. When the next action would be another broad grep, another nearby `sed`, or a test search, prefer writing `/tmp/memos_edit.py` for the smallest source behavior change already supported by the prompt and current source.", "6. If `git diff` is empty, the task is not complete.", "", "### Hard gates", "1. Treat the task repository as `/testbed`; never switch to `/repo` unless the current prompt explicitly says so.", "2. Use the exact current `WRAPPER_PATH` from the task prompt. Do not reuse a hard-coded `/tmp/...-exec` path from memory; retries can change it.", "3. Use copy-paste-safe wrapper calls matching the task prompt: double quotes around the `tmux-run` command, then single quotes inside that command when a search pattern needs quoting.", `   - Inspect: \`${shellPrefix} 10\``, "   - Good identifier grep: `WRAPPER_PATH tmux-run \"cd /testbed && grep -n combine path/to/file.py\" 10`. Search one bare identifier, not a phrase with spaces.", "   - Good literal grep: `WRAPPER_PATH tmux-run \"cd /testbed && grep -n 'literal_pattern_without_spaces' path/to/file.py\" 10`", "   - Good sed: `WRAPPER_PATH tmux-run \"cd /testbed && sed -n '120,180p' path/to/file.py\" 10`", "   - Good poll: `WRAPPER_PATH tmux-run \"\" 10` only after a previous command reported still running.", "   - Good script/status/diff: `WRAPPER_PATH tmux-run \"cd /testbed && python /tmp/memos_edit.py\" 10`, `WRAPPER_PATH tmux-run \"cd /testbed && git diff\" 10`, `WRAPPER_PATH tmux-run \"cd /testbed && git status --porcelain\" 10`.", "4. Never put raw inner double quotes inside the `tmux-run \"...\"` command. Bad: `grep -n \"pattern\" file`; good: `grep -n 'pattern' file`.", "5. Every non-poll `tmux-run` command must start with `cd /testbed &&`. Never run `python /tmp/...`, `git diff`, `git status`, or check scripts without that prefix.", "6. Do not use complex shell forms inside `tmux-run`: no inline `python - <<`, no heredoc, no `cat >`, no `tee`, no `sed -i`, no `perl -pi`, no `apply_patch`, no `patch`, no `git apply`, no `sh -lc`, no `bash -lc`, no shell pipes (`|`), no nested `tmux-run`, and no empty `tmux-run` except polling a still-running command.", "7. Never grep for a phrase containing whitespace inside `tmux-run` such as `grep -n \"def combine\"`, `grep -n '^    def combine'`, or `rg -n \"def combine\"`; those split the host wrapper arguments and can leave a stale running grep. Grep the bare identifier (`combine`) or inspect a line range with `nl`/`sed`.", `8. Use outer \`write\` for target files or temporary scripts, e.g. \`${writePrefix}\`. The wrapper path is host-side, so do not inspect it from \`/testbed\` or call it inside tmux-run.`, "9. A `WRAPPER_PATH write` heredoc is literal. Do not put shell substitutions like `$(sed ...)`, command output placeholders, line numbers, or diff markers (`+`/`-` prefixes) into the file content.", "10. Do not finish by saying the bug is already fixed. For verifier-scored repair tasks, a valid completion needs a non-empty source `git diff`; if `git diff` is empty, continue source investigation instead of `TASK_COMPLETE`.", "", "### Safe edit loop", `1. For any multi-line source edit, first create \`${editScriptPrefix}\` with the outer wrapper.`, "2. The edit script should use `Path('/testbed/...')`, exact `old`/`new` replacement strings copied from inspected source, `assert old in text`, and `text.replace(old, new, 1)`.", "3. Run the script with `WRAPPER_PATH tmux-run \"cd /testbed && python /tmp/memos_edit.py\" 10`.", "4. If `OLD block not found`, inspect the actual block with simple `nl`/`sed`/single-pattern `grep`, then rewrite the temporary edit script with outer `write`. Do not fall back to inline heredoc, `sed -i`, or broad rewrites.", "5. If a command syntax error mentions an unclosed quote/heredoc/bracket, stop using that command shape immediately and switch to the outer-write temporary script pattern.", "6. If a poll or script run repeats stale source text from an earlier command instead of showing the new command output, stop polling. Run one fresh `cd /testbed && git diff -- <target-file>` or `cd /testbed && git status --porcelain`; if that also repeats stale text, rewrite the edit/check script and execute it with the exact `cd /testbed &&` prefix.", "", "### Search and test discipline", "1. Prefer POSIX tools (`grep`, `find`, `nl`, `sed`) because `rg` may be unavailable. Use simple single-token searches; avoid shell pipelines (`|`), phrase searches with spaces, and alternation (`\\|`) in wrapper command strings because the evaluation allowlist is conservative.", "2. If a Bug hint digest or Visible bug clue digest is present, inspect the target source file at most twice, then apply the minimal source edit before searching for regression-test locations.", "3. If bug hints contain a candidate source diff or visible bug clues contain a current -> expected expression, apply the minimal source fix before running full tests or searching broadly. Existing tests come after the source edit.", "4. Held-out verification scores the source patch. Do not create or keep searching for new regression tests after existing targeted tests pass; run `git diff` and finish.", "5. When a candidate diff is present, do not generalize the same idea to other similar call sites, files, tests, docs, or helper functions unless the candidate diff explicitly touches them. Extra edits outside the candidate hunks can break existing behavior checks.", "6. Verify with the project's native targeted tests: Django usually uses `python tests/runtests.py ...`; SymPy usually uses `python bin/test ...`; only use `python -m pytest` after confirming pytest exists. Do not install pytest just to run one benchmark task.", "7. Before `TASK_COMPLETE`, run `git diff`, confirm the patch is non-empty, and check it does not delete unrelated files or tests.", "8. If `git diff` is empty, tests or reproduction output are not enough to finish. Keep narrowing the source behavior and make the smallest source edit.", "9. If the Bug hint digest contains a required `+` checklist or expected added-line count, it is a completion gate: compare `git diff` against it and continue editing until every listed source line/effect is present. Targeted or broad tests passing is not sufficient when the checklist is incomplete.", "10. Convergence budget: after locating the visible target class/function and one neighboring same-family implementation, either write the minimal source patch or run one narrow reproduction. Do not keep doing broad grep/test searches without producing a patch.", "11. For operation-reduction or optimizer bugs, inspect the named operation classes plus the base reducer/optimizer contract, then patch the missing reduction method before broad test discovery.", "12. If a Visible bug clue digest names identifiers, do not run `ls` or `pwd` first; the first command should grep one exact visible identifier.");
    return protocol.join("\n");
}
function inferRepairSourceRoots(text) {
    const lower = String(text ?? "").toLowerCase();
    if (lower.includes("django/django"))
        return "django tests";
    if (lower.includes("sympy/sympy"))
        return "sympy";
    if (lower.includes("scikit-learn/scikit-learn"))
        return "sklearn";
    if (lower.includes("matplotlib/matplotlib"))
        return "lib";
    return ".";
}
function renderImmediateRepairActionGate(text) {
    ...
    if (renderCleanedDataDefaultDigest(visible)) {
        patternLines.push("Visible pattern: cleaned_data/default override. After reading the construct/assignment guard, edit that guard now: add a cleaned-value emptiness condition to the existing default+omitted-data `continue`.");
    }
    if (renderFactorGroupingDigest(visible)) {
        patternLines.push("Visible pattern: factor-list multiplicity aggregation. After reading the public wrapper and list assembly point, edit the wrapper/list assembly path before tests.");
    }
    if (renderRandomStateDigest(visible)) {
        patternLines.push("Visible pattern: shuffle/random_state propagation. After reading the splitter loop, edit RNG normalization/propagation before test discovery or ad-hoc reproductions; if the loop passes one raw `random_state` into repeated per-class/per-stratum splitters, normalize it once to the repository RNG object and reuse that object.");
    }
    if (renderModelFieldComparisonDigest(visible)) {
        patternLines.push("Visible pattern: model-attached object comparison. After reading equality/hash/order methods, edit a stable primitive model-key helper before test discovery; do not place raw model classes/objects inside the ordering/hash tuple.");
    }
    if (renderEnumValueCastingDigest(visible)) {
        patternLines.push("Visible pattern: enum/choices value casting. After reading the shared Choices/enum base, edit the common value representation path before test discovery.");
    }
    if (renderSingleAliasDeleteDigest(visible)) {
        patternLines.push("Visible pattern: single-alias SQL fast path. After reading the alias-count branch, edit base alias initialization before test discovery.");
    }
    return [
...
function renderRepairVisibleBugDigest(text, sourceRoots) {
    ...
    if (identifiers.length === 0 && replacements.length === 0 && !patternDigest)
        return null;
...
function extractVisibleBugIdentifiers(text) {
    const buckets = [
    ...
    const addToken = (raw) => {
        ...
        if (!token || seen.has(token))
            return;
        if (!isUsefulVisibleBugToken(token))
            return;
        seen.add(token);
        out.push(token);
    };
...
function normalizeVisibleBugToken(token) {
    return token
        .replace(/[.,;:)\]]+$/g, "")
        .replace(/^[([{\s]+/g, "")
        .trim();
}
function isUsefulVisibleBugToken(token) {
    if (token.length < 3 || token.length > 120)
        return false;
    if (/^https?:/i.test(token))
        return false;
    if (/^(?:Bug|Description|Patch|Django|Reply|TASK_COMPLETE)$/i.test(token))
        return false;
    return (/[_.%/]/.test(token) ||
...
function renderVisibleBugPatternDigest(text, identifiers) {
    ...
    const asksForReduction = /\b(?:reduce|reduction|optimi[sz](?:e|ing|ation)|coalesce|collapse|cancel)\b/i
        .test(text);
    const mentionsOperations = /\boperations?\b/i.test(text) ||
        identifiers.some((id) => splitOperationToken(id));
    const addLike = identifiers.filter((id) => /^(?:Add|Create|Insert|Set)[A-Z]/.test(id));
    const removeLike = identifiers.filter((id) => /^(?:Remove|Delete|Drop|Unset)[A-Z]/.test(id));
...
function renderCleanedDataDefaultDigest(text) {
    if (!/\bcleaned_data\b/i.test(text) ||
...
function renderEnumValueCastingDigest(text) {
    if (!/\b(?:TextChoices|IntegerChoices|Choices)\b/.test(text) ||
        !/\benum\b/i.test(text) ||
...
function renderModelFieldComparisonDigest(text) {
...
function renderRandomStateDigest(text) {
    if (!/\b(?:StratifiedKFold|stratification|stratum|strata|class(?:'s)? samples)\b/i.test(text) ||
...
function renderSingleAliasDeleteDigest(text) {
    if (!/\bdelete\(\)/i.test(text) ||
...
function renderAliasPrefixCollisionDigest(text) {
    if (!/\balias_prefix\b/i.test(text) ||
...
function renderNaturalKeyDatabaseDigest(text) {
    if (!/\bloaddata\b/i.test(text) ||
...
function renderFactorGroupingDigest(text) {
    if (!/\b(?:factor|factors|factorization|sqf|sqf_list)\b/i.test(text) ||
...
function renderRepairHintDigest(text) {
...
function renderExactReplacementScriptPattern(target) {
    return [
