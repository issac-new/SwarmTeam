...
export function buildQueryWithExtract(ctx, extract, opts) {
    return finalize(rawQueryText(ctx, opts), extract);
}
function applyIrQueryFocus(raw, domain) {
    if (!isIrDomain(domain))
        return raw;
    return focusIrRetrievalQuery(raw).text;
}
export function rawQueryText(ctx, opts) {
    switch (ctx.reason) {
        case "turn_start": {
...
// ─── Helpers ────────────────────────────────────────────────────────────────
function finalize(raw, llmExtract) {
    ...
    const patternTerms = keywordPatternTerms.length > 0 ? keywordPatternTerms : extractPatternTerms(trimmed);
...
function normalizeRetrievalExtract(raw, llmExtract) {
...
function isUsableQueryVecText(text) {
...
function sanitizeKeywordList(keywords) {
    ...
    for (const item of keywords) {
        ...
        seen.add(normalized);
        out.push(keyword);
        if (out.length >= MAX_KEYWORD_TOKENS)
            break;
    }
    return out;
}
...
export function isSoftwareRepairPrompt(text) {
...
export function extractRepairTaskSection(text, heading) {
    ...
    const match = text.match(new RegExp(`(?:^|\\n)\\s*##\\s*${escapedHeading}\\b\\s*([\\s\\S]*?)(?=\\n\\s*(?:##\\s+|STRICT RULES:|Reply TASK_COMPLETE\\b)|$)`, "i"));
    return (match?.[1] ?? "").trim();
}
