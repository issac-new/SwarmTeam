...
export function toPacket(input) {
    ...
    const rendered = prependIrRenderPrompt(renderWholePacket(snippets, input.reason, {
        skillMode,
        decisionGuidance: input.decisionGuidance,
        standaloneMathFinalAnswer,
        taskProtocol: input.taskProtocol,
    }), input.reason, input.domain);
    const packet = {
        reason: input.reason,
        snippets,
        rendered,
...
function renderSkill(c, opts) {
    if (opts.skillMode === "full") {
        const body = truncate(`Skill: ${c.skillName}\n` + c.invocationGuide.trim(), FULL_SKILL_SNIPPET_BODY_CHARS);
        return {
            refKind: "skill",
            refId: c.refId,
            title: c.skillName,
...
function renderTrace(c) {
    ...
    const body = withToolFollowUp(truncate(parts.join("\n"), MAX_SNIPPET_BODY_CHARS), `→ call \`memos_get(id="${c.refId}", kind="trace")\` for the full turn`);
...
function renderEpisode(c) {
    ...
    const body = withToolFollowUp(truncate(stripEpisodePromptMetrics(c.summary), MAX_SNIPPET_BODY_CHARS), `→ If this past task matches the current task type, call \`memos_timeline(episodeId="${c.refId}")\` BEFORE your first tool call to see what tools were used and what succeeded.`);
    const when = formatMemoryTimestamp(c.ts);
    return {
        refKind: "episode",
        refId: c.refId,
...
function renderExperience(c) {
    const trigger = c.trigger && c.trigger.trim() !== c.title.trim()
        ? `Trigger: ${c.trigger}`
        : null;
    const parts = [
    ...
    return {
        ...
        refId: c.refId,
        title: c.title,
        body: withToolFollowUp(truncate(parts.join("\n"), MAX_SNIPPET_BODY_CHARS), `→ call \`memos_get(id="${c.refId}", kind="policy")\` for the full policy details before relying on this experience`),
    };
}
function renderExperienceUseHint(c) {
...
function renderWorldModel(c) {
    const body = withToolFollowUp(truncate(`World model: ${c.title}\n${c.body}`, MAX_SNIPPET_BODY_CHARS), `→ call \`memos_get(id="${c.refId}", kind="world_model")\` for the full environment knowledge`);
    return {
        refKind: "world-model",
        refId: c.refId,
        title: c.title,
...
function renderWholePacket(snippets, reason, opts) {
    ...
    if (snippets.length === 0 && !guidanceBlock && !taskProtocol)
        return "";
...
const TASK_PROTOCOL_HEADER = "# Current task protocol and recalled memories\n\n" +
...
function prependIrRenderPrompt(rendered, _reason, domain) {
    if (!isIrDomain(domain))
        return rendered;
