...
function rejectAll(ranked, outcome, sufficient) {
    return {
        kept: [],
        dropped: [...ranked],
        outcome,
...
function fallbackCap(ranked, deps, outcome) {
...
function fallbackMaxKeep(deps) {
    return Math.max(0, deps.config.llmFilterFallbackMaxKeep ?? Math.min(deps.config.llmFilterMaxKeep, 4));
}
...
function describeExperience(ex, bodyChars) {
    const headParts = [
