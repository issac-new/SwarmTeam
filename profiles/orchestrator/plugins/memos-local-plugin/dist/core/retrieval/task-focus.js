 ...
 */
export function extractQuestionSection(raw) {
    const text = raw.trim();
    if (!text)
        return null;
    ...
    for (let i = start; i < lines.length; i += 1) {
        const line = lines[i];
        if (bodyLines.length > 0 && /^#{1,3}\s+\S/.test(line.trim()))
            break;
        bodyLines.push(line);
    }
...
export function isIrEvalPrompt(raw) {
    const text = raw.trim();
    if (!text)
        return false;
    if (!IR_EVAL_MARKERS_RE.test(text))
        return false;
 ...
 */
export function focusIrRetrievalQuery(raw) {
    const text = raw.trim();
    if (!text)
        return { text: "", method: "passthrough" };
    if (!isIrEvalPrompt(text))
        return { text, method: "passthrough" };
    const question = extractQuestionSection(text);
    if (question)
        return { text: question, method: "question_section" };
