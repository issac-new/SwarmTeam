...
export function backprop(input) {
    ...
    const effectiveAlpha = input.traces.map((trace) => alphaFromTrace(trace));
    const weights = new Array(input.traces.length).fill(0);
    let sumW = 0;
    ...
    for (let i = 0; i < input.traces.length; i++) {
        ...
        if (!reflection || !reflection.trim())
            fallbackAlphaCount += 1;
        else if (!KNOWN_REFLECTION_LABELS.has(reflection.trim()))
