...
export function buildTaskSummary(input) {
    ...
    const missionText = (typeof episode.meta?.canonicalGoal === "string" && episode.meta.canonicalGoal.trim().length > 0)
...
function buildExecutionOutcome(traces) {
    let totalToolCalls = 0;
    let successCount = 0;
    let errorCount = 0;
    const sorted = [...traces].sort((a, b) => a.ts - b.ts);
    let lastTraceWithTools = null;
    for (const trace of sorted) {
        const calls = (trace.toolCalls ?? []);
        if (calls.length > 0)
            lastTraceWithTools = trace;
    ...
    const calls = (lastTraceWithTools.toolCalls ?? []);
    const lastCall = calls[calls.length - 1];
    const lastToolResult = lastCall.errorCode ? "ERROR" : "SUCCESS";
    return {
        totalToolCalls,
        successCount,
        errorCount,
...
function formatExecutionOutcome(traces) {
