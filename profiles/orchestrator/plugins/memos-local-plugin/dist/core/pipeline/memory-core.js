...
const TURN_START_TASK_PROTOCOL_FALLBACK_TIMEOUT_LABEL = "turn_start_task_protocol_fallback_timeout";
...
export function createMemoryCore(handle, home, pkgVersion, options = {}) {
    ...
    function resolveOpenEpisodeId(sessionId) {
        ensureLive();
        return handle.resolveOpenEpisodeId(sessionId);
    }
    function isEpisodeWritable(episodeId) {
        ensureLive();
    ...
    function reconcileEpisodeId(sessionId, candidate) {
        ensureLive();
        ...
        if (candidate && isEpisodeWritable(candidate))
            return candidate;
        return undefined;
    }
    ...
    async function onTurnStart(turn) {
        ...
        activeNamespace = ns;
        const canUseProtocolFallback = turn.contextHints?.__memosReadOnlyTurnStart === true || Boolean(turn.episodeId);
        const protocolFallbackPacket = canUseProtocolFallback
            ...
            : null;
        const protocolFallbackResult = protocolFallbackPacket
            ? {
                query: {
                    agent: turn.agent,
                    namespace: ns,
        ...
        try {
            ...
            if (protocolFallbackResult && protocolFallbackPacket) {
                ...
                try {
                    return await withTimeout(fullRetrieval, TURN_START_TASK_PROTOCOL_FALLBACK_TIMEOUT_MS, TURN_START_TASK_PROTOCOL_FALLBACK_TIMEOUT_LABEL);
                }
                catch (err) {
    ...
    function collectInjectedPolicyIds(hits) {
        const ids = new Set();
        for (const hit of hits) {
            if (hit.refKind !== "experience")
                continue;
            if (typeof hit.refId !== "string" || !hit.refId.trim())
                continue;
            ids.add(hit.refId);
        }
    ...
    async function searchMemory(query) {
        ...
        const deps = {
            ...
            config: {
                ...
                readOnlyInjectionProfile: query.injectionProfile ?? effectiveReadOnlyInjectionProfile(baseDeps.config),
            },
            namespace: ns,
    ...
    async function getTrace(id, namespace, opts) {
        ensureLive();
        if (namespace)
    ...
    async function updateTrace(id, patch, opts) {
        ensureLive();
        const existing = handle.repos.traces.getById(id);
        if (!existing || (!opts?.includeAllNamespaces && !ownedByCurrent(existing)))
            return null;
    ...
    async function deleteTrace(id, opts) {
        ensureLive();
        const existing = handle.repos.traces.getById(id);
        if (!existing || (!opts?.includeAllNamespaces && !ownedByCurrent(existing)))
    ...
    async function deleteTraces(ids, opts) {
        ensureLive();
        let deleted = 0;
    ...
    async function shareTrace(id, share, opts) {
        ensureLive();
        const existing = handle.repos.traces.getById(id);
        if (!existing || (!opts?.includeAllNamespaces && !ownedByCurrent(existing)))
            return null;
