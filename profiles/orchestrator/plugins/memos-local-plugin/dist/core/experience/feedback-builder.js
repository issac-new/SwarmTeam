...
export async function runFeedbackExperience(input, deps) {
    ...
    if (deps.llm && (input.trace || input.episode)) {
        episodeContext = buildEpisodeContext(input.episode, input.trace ?? null, deps.repos, text);
    ...
    const provisionalPolicyId = ids.policy();
...
function logRefineFallback(log, event, ids) {
    if (!event)
        return;
    const level = event.reason === "llm_disabled" ? "info" : "warn";
...
async function buildDraft(args) {
    ...
    if (args.llm && (args.trace || args.episode)) {
        try {
            const episodeContext = args.episodeContext ?? buildEpisodeContext(args.episode, args.trace, args.repos, args.text);
...
function deriveFeedbackSimilarityKeyFromPolicy(row) {
    const text = [
        row.title,
        row.trigger,
        row.procedure,
...
function feedbackSourceKind(raw, verifierMeta, lower) {
    const src = rawSource(raw);
    if (src && /evaluator|evoagentbench|benchmark|gateway/i.test(src))
        return "evaluator";
    if (src && /verifier|verification/i.test(src))
        return "verifier";
    if (verifierMeta || /\bverifier\b|\bverification\b/.test(lower))
        return "verifier";
    return "unknown";
}
function rawSource(raw) {
    let obj = raw;
...
function outcomeKindOf(outcome, polarity) {
    if (outcome === "pass")
        return "pass";
    if (outcome === "fail")
        return "fail";
...
function issueKindOf(type, polarity) {
...
function extractTaskKind(text) {
    const lower = text.toLowerCase();
    if (/\bsec\s*13f\b/.test(lower))
        return "sec_13f";
    if (/\bfft\b|autocorrelation|triplets?/.test(lower))
        return "array_triplets";
    const tokens = lower
        .split(/[^a-z0-9\u4e00-\u9fff]+/u)
        .map((t) => t.trim())
...
function isMetaOnlyFeedback(text, verifierMeta) {
    const lower = text.toLowerCase();
    if (/\bheartbeat_ok\b/.test(lower))
        return true;
...
function isMetaOnlyDerivedGuidance(parts) {
...
function findSimilarPolicy(draft, vec, mergeFamily, deps) {
    if (!vec)
        return null;
    if (isSimilarityBannedSource(draft.similarityKey?.sourceKind))
        return null;
    ...
    for (const hit of hits) {
        ...
        if (minScore == null || hit.score < minScore)
            continue;
        return row;
    }
    return null;
}
function feedbackSimilarityThreshold(draft, row) {
    ...
    if (incoming.sourceKind !== existing.sourceKind)
        return null;
    if (incoming.outcomeKind !== existing.outcomeKind)
        return null;
    if (incoming.taskKind && existing.taskKind && incoming.taskKind !== existing.taskKind)
        return null;
    if (incoming.issueKind && existing.issueKind && incoming.issueKind !== existing.issueKind)
        return null;
...
function isSimilarityBannedSource(sourceKind) {
 ...
 */
function buildEpisodeContext(episode, currentTrace, repos, feedbackText) {
    ...
    const contextParts = selected.kept.map((item) => formatFeedbackTraceTurn(item.idx + 1, item.trace));
...
function classifyLlmFallbackReason(err) {
    if (MemosError.is(err)) {
        if (err.code === ERROR_CODES.LLM_TIMEOUT)
            return "llm_timeout";
        if (err.code === ERROR_CODES.LLM_OUTPUT_MALFORMED)
