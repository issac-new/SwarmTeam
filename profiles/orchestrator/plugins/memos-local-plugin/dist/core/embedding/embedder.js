        ...
        for (const [role, list] of byRole.entries()) {
            for (let start = 0; start < list.length; start += batchSize) {
                ...
                try {
                    ...
                    const errMessage = err instanceof MemosError
                    ...
                    throw err instanceof MemosError
                        ? err
                        : new MemosError(ERROR_CODES.EMBEDDING_UNAVAILABLE, `${provider.name} failed: ${err.message ?? String(err)}`, { provider: provider.name });
                }
                if (raw.length !== texts.length) {
                    throw new MemosError(ERROR_CODES.EMBEDDING_UNAVAILABLE, `${provider.name} returned ${raw.length} vectors for ${texts.length} inputs`, { provider: provider.name });
                }
