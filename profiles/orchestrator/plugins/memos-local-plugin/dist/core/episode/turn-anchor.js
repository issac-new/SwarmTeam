...
export function readAnchorTurnId(episode) {
...
export function resolveAnchorTurnId(episode) {
    return readAnchorTurnId(episode) ?? fallbackAnchorTurnId(episode);
}
function fallbackAnchorTurnId(episode) {
    const firstUser = episode.turns.find((t) => t.role === "user");
    if (firstUser)
        return firstUser.ts;
    return episode.startedAt;
}
export function anchorTurnIdFromFirstUserTs(ts) {
    return ts;
}
export function liteCaptureTurnCursor(episode) {
...
export function pickTurnId(meta, fallbackTs) {
    const raw = meta?.turnId;
    return typeof raw === "number" && Number.isFinite(raw) ? raw : fallbackTs;
}
export function episodeAlreadyHasUserTextTrace(existing, anchorTurnId) {
    return existing.some((r) => r.turnId === anchorTurnId && (r.userText ?? "").trim().length > 0);
}
export function stripRepeatedEpisodeUserText(rows, existing, _anchorTurnId) {
    const seenUserTexts = new Set(existing
        .map((row) => (row.userText ?? "").trim())
        .filter((text) => text.length > 0));
    if (seenUserTexts.size === 0)
        return rows;
    return rows.map((row) => {
...
export function stepIdentitySignature(step, anchorTurnId) {
    // Signature key can stay episode-stable (anchor) even when persisted
    // row `turnId` is per-segment for viewer ordering/grouping.
    const turnId = typeof anchorTurnId === "number" && Number.isFinite(anchorTurnId)
        ? anchorTurnId
...
//# sourceMappingURL=turn-anchor.js.map