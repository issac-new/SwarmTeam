            ...
            if (!llmDisabled && opts.llm) {
                try {
                    ...
                    if (result.relation === "new_task" &&
                        ...
                        try {
                            const arb = await withTimeout(callArbitration(opts.llm, input), timeoutMs, "relation.arbitration.timeout");
...
async function callLlm(llm, input) {
...
async function callArbitration(llm, input) {
    const userContent = [
