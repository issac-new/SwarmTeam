...
export async function batchScoreReflections(llm, inputs, opts) {
    ...
    const rsp = await llm.completeJson([
        ...
        phase: opts.phase,
        schemaHint: '{"scores": [{"idx": int, "relevance": "IRRELEVANT|RELATED|PIVOTAL", "reason": "str"}]}',
    ...
    const scores = inputs.map((input, i) => {
        ...
        const label = socialOnly ? "IRRELEVANT" : baseLabel;
...
function validateBatchPayload(v, expected) {
    ...
    for (const entry of o.scores) {
        ...
        if (entry.relevance !== "IRRELEVANT" && entry.relevance !== "RELATED" && entry.relevance !== "PIVOTAL") {
            throw new MemosError(ERROR_CODES.LLM_OUTPUT_MALFORMED, "batch reflection: relevance must be IRRELEVANT/RELATED/PIVOTAL", { idx: entry.idx, got: entry.relevance });
        }
    }
}
...
function alphaForReflection(label) {
    if (label === "PIVOTAL")
        return 1;
...
function mapRawRelevance(relevance) {
    if (relevance === "PIVOTAL")
        return "PIVOTAL";
...
function isSocialOnlyStep(step) {
    if (step.toolCalls.length > 0)
        return false;
    const combined = `${step.userText}\n${step.agentText}\n${step.agentThinking ?? ""}`.toLowerCase();
    if (!combined.trim())
        return false;
    const socialPattern = /(谢谢|感谢|辛苦|棒|很好|很对|厉害|夸奖|客气|不用谢|再见|拜拜|你好|您好|早上好|晚上好|thank(s| you)?|appreciate|great job|well done|awesome|nice|you're welcome|no problem|bye|goodbye|hello|hi)/i;
