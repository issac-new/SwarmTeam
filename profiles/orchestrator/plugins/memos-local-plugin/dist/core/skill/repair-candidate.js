...
export function selectRepairEvidenceAnchors(policy, deps) {
...
function loadPolicyEpisodeTraces(policy, repos) {
    const out = [];
    const seen = new Set();
    for (const episodeId of policy.sourceEpisodeIds ?? []) {
        for (const trace of loadEpisodeTraces(episodeId, repos)) {
            if (seen.has(trace.id))
                continue;
            seen.add(trace.id);
            out.push(trace);
