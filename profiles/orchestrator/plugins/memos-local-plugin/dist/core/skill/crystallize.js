...
import { languageSteeringLine, } from "../llm/prompts/index.js";
...
export async function crystallizeDraft(input, deps) {
    ...
    const promptDef = mode === "rebuild" ? SKILL_REBUILD_PROMPT : SKILL_CRYSTALLIZE_PROMPT;
    ...
    try {
        const rsp = await llm.completeJson([
            ...
            schemaHint: mode === "rebuild" ? "skill-rebuild.v3" : "skill-crystallize.v6",
        });
        ...
        const changedSections = asStringArray(rsp.value.changed_sections ?? rsp.value.changedSections);
...
function packPrompt(input, config, mode) {
    ...
    const incremental = (input.incrementalEvidence ?? [])
        ...
        .map(mapTrace);
    const evidenceTools = Array.from(extractToolNames([
    ...
    if (mode === "rebuild" && input.existingSkill) {
        ...
        payload.existing_skill_snapshot = existingSkillSnapshot(proc, input.existingSkill.name);
        payload.rebuild_level = input.rebuildLevel ?? "L1";
        payload.repair_rename_allowed = Boolean(input.renameAllowed);
        if (incremental.length > 0)
            payload.incremental_evidence = incremental;
    }
...
function normaliseDraft(raw, input) {
    ...
    const lockName = input.mode === "rebuild" && input.existingSkill && !input.renameAllowed
    ...
    // there.
    const emptyFallback = input.mode === "rebuild" && input.renameAllowed && input.existingSkill
        ? input.existingSkill.name
        : `skill_${input.policy.id.slice(-6)}`;
    const name = lockName ?? normalizeSkillName(rawName || emptyFallback);
    const retrievalBlurb = sanitizeDerivedMarkdown(raw.retrieval_blurb ?? raw.retrievalBlurb ?? "");
    const triggerContext = sanitizeDerivedText(raw.trigger_context ?? raw.triggerContext ?? "");
    ...
    const steps = asArray(raw.steps).map(coerceStep).filter(Boolean);
