...
import { gatherCounterExamples, gatherEvidence, gatherIncrementalEvidence, } from "./evidence.js";
import { mergeRebuildDraft, procedureFromSkillRow } from "./merge.js";
import { computeRebuildLevel } from "./rebuild-level.js";
...
export async function runSkill(input, deps) {
    ...
    for (const decision of eligibility.decisions) {
        ...
        const incremental = decision.action === "rebuild" && decision.existingSkill
            ? gatherIncrementalEvidence(decision.policy, decision.existingSkill, {
                repos,
                config,
            })
            : { traces: [], poolAfterDedupe: 0 };
        const rebuildMeta = decision.action === "rebuild" && decision.existingSkill
            ? computeRebuildLevel({
        ...
        const existingProc = decision.existingSkill
            ...
            : null;
        const renameAllowed = Boolean(decision.action === "rebuild" &&
        ...
        if (renameAllowed) {
            const existingNames = new Set(repos.skills.list({ limit: 500 }).map((s) => s.name));
            existingNames.delete(decision.existingSkill.name);
            draft.name = uniquifySkillName(normalizeSkillName(draft.name || decision.existingSkill.name), existingNames);
        }
        const tVerify = nowMs();
        const verdict = verifyDraft({ draft, evidence: evidence.traces.map((a) => a.trace) }, { log: log.child({ channel: "core.skill.verifier" }) });
        ...
        if (renameAllowed && row.procedureJson && typeof row.procedureJson === "object") {
            row.procedureJson.graduatedFromRepairName = true;
        }
...
async function runCrystallize(args, deps) {
    const { policy, evidence, counterExamples, skillsByPolicy, action, existingSkill } = args;
