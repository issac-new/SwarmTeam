...
export function normalizeSkillName(raw, fallback = "generic_task_apply") {
...
export function buildStructuredName(input) {
    ...
    return normalizeSkillName([domain || "generic", task || "task", action || "apply"].join("_"), input.fallback ?? "generic_task_apply");
}
export function deriveNameFromText(title, hint) {
...
export function uniquifySkillName(base, existing) {
    ...
    for (let i = 2; i < 200; i++) {
        ...
        if (!existing.has(fitted))
            return fitted;
    }
    return fitName(normalizedBase, 44) + "_x";
}
function enforceSkeleton(parts) {
    const out = parts.filter((p) => p.length > 0);
    if (out.length >= 3)
        return out;
    if (out.length === 2)
        return [out[0], out[1], "apply"];
    if (out.length === 1)
        return [out[0], "task", "apply"];
...
function fitName(name, max) {
    ...
    if (parts.length < 3)
        return name.slice(0, max).replace(/_+$/g, "");
...
function findAction(words) {
    for (const w of words) {
        ...
        if (ACTION_WORDS.includes(action))
            return action;
    }
    return null;
}
