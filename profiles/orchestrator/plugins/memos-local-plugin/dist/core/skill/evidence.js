...
export function gatherEvidence(policy, deps) {
    ...
    const deduped = dedupeTracesBySignature(evidencePool, epMeta, (t, p) => scoreTrace(t, policy) - scoreTrace(p, policy));
...
function dedupeTracesBySignature(traces, epMeta, prefer) {
    const best = new Map();
