...
function headlessRegistryPath(agent) {
    const agentHome = agent === "hermes" ? ".hermes" : ".openclaw";
    return path.join(process.env.HOME ?? "/tmp", agentHome, "memos-plugin", "daemon", "headless-bridges.json");
}
function removeHeadlessRegistryEntry(agent) {
    const registryPath = headlessRegistryPath(agent);
    try {
        const raw = JSON.parse(fs.readFileSync(registryPath, "utf8"));
        const bridges = Array.isArray(raw?.bridges) ? raw.bridges : [];
        const next = bridges.filter((entry) => {
            if (!entry || typeof entry !== "object")
                return false;
            return Number(entry.pid) !== process.pid;
        });
        if (next.length === bridges.length)
            return;
...
function headlessIdleTimeoutMs() {
    const raw = process.env.MEMOS_HEADLESS_BRIDGE_IDLE_TTL_SEC;
    if (!raw)
        return HEADLESS_BRIDGE_IDLE_TTL_MS;
...
async function main() {
    ...
    const shutdown = async (sig) => {
        ...
        removeOwnedPidFile();
        if (args.noViewer)
            removeHeadlessRegistryEntry(args.agent);
        if (viewer) {
            try {
                await viewer.close();
            }
