...
function memoryAddDisabledFromConfig(config) {
    const memoryAdd = config?.memory_add;
    if (!memoryAdd || typeof memoryAdd !== "object" || Array.isArray(memoryAdd)) {
        return false;
    }
    return memoryAdd.enabled === false;
}
async function createRuntime(api, runtimeLock, opts = { readOnly: false }) {
...
function register(api) {
    ...
    if (memoryAddDisabledFromConfig(api.pluginConfig)) {
        api.logger.info("memos-local: memory_add disabled; allowing concurrent read-only runtime");
    }
