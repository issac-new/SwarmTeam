...
function traceBodyText(trace, bodyCap) {
    const body = clip(trace.agentText, bodyCap);
    const toolText = (trace.toolCalls?.length ?? 0) > 0
...
export function registerOpenClawTools(api, opts) {
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Expand a similar past task into its step-by-step execution trace — showing which tools " +
