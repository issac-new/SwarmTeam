...
const AGENT_END_BINDING_CAP_MS = Math.min(60_000, BEFORE_PROMPT_SOFT_TIMEOUT_MS * 5);
...
export function createOpenClawBridge(opts) {
    ...
    const readOnly = truthyEnv("OPENCLAW_MEMOS_READONLY") || truthyEnv("MEMOS_READONLY");
    ...
    const sessionTurnGeneration = new Map();
    ...
    function bindingStillLive(binding) {
    ...
    function commitEpisodeId(binding, pkt) {
        if (!bindingStillLive(binding))
            return;
        const candidate = pkt.query?.episodeId;
        if (!candidate)
            return;
        const routedSessionId = (pkt.query?.sessionId ?? binding.sessionId);
        if (opts.core.episodeExists(candidate)) {
            ...
            if (fallback)
                binding.episodeId = fallback;
        }
        if (routedSessionId !== binding.sessionId) {
            for (const key of turnBindingKeys(routedSessionId, { runId: binding.runId }, binding.userText)) {
    ...
    function rememberEpisodeBinding(input) {
        if (!input.turnStart && input.episodeId === undefined) {
            throw new Error("rememberEpisodeBinding: turnStart required when episodeId is pending");
        }
    ...
    function findEpisodeBinding(sessionId, ctx, userText, mode = "legacy") {
    ...
    async function resolveEpisodeIdForTurnEnd(sessionId, ctx, userText) {
        if (!ctx.runId) {
    ...
    async function handleBeforePrompt(event, ctx) {
        ...
        try {
            ...
            const routedEpisodeId = binding.episodeId ??
                packet?.query.episodeId;
            const renderedBlock = renderContextBlock(packet, { hintWhenEmpty: false });
            const { block, truncated } = capContextBlock(renderedBlock);
            const durationMs = now() - startedAt;
            opts.log.info("memos.onTurnStart", {
    ...
    async function handleAgentEnd(event, ctx) {
        ...
        try {
            ...
            if (isSubagentAnnouncement) {
                const probe = findEpisodeBinding(sessionId, ctx, turn.userText, "strict_run");
            ...
            const resolved = await resolveEpisodeIdForTurnEnd(sessionId, ctx, turn.userText);
            if (!resolved) {
                opts.log.warn("memos.agent_end.skipped", {
