...
- `popular-web-designs` — 54 real design systems (Stripe, Linear, Vercel) as HTML/CSS. Use for visual vocabulary when the user wants a known brand's look.
- `design-md` — Google's DESIGN.md token spec format. Use when the deliverable is a formal token spec file rather than a rendered artifact.
- `excalidraw` — Hand-drawn Excalidraw JSON diagrams for architecture, flow, sequence diagrams.
- `architecture-diagram` — Dark-themed SVG architecture/cloud/infra diagrams as HTML.

**See also:** `references/multi-page-product-suite.md` for the shared-component architecture pattern used when building products with multiple pages/screens.
...
### Multi-Page Product Suite Pattern

When the user asks for a product with multiple pages/screens (e.g., "a dashboard with settings, profile, and analytics pages"):

1. **Extract shared components first** — before building any page, define a shared CSS/JS foundation:
   - `design-tokens.css` — CSS variables for colors, spacing, typography, shadows
   - `layout.css` — shared layout scaffolding (sidebar, nav, main content areas, responsive breakpoints)
   - `components.css` — reusable components (buttons, cards, inputs, tags, badges, toasts, modals)
   - `app.js` — shared JavaScript (routing helpers, theme management, global search, toast notifications, keyboard shortcuts)
   - `api.js` — mock API layer for consistent data across pages

2. **Link pages to shared resources** — each page HTML should `<link>` to the shared CSS and `<script>` to the shared JS, using relative paths (`../css/design-tokens.css`).

3. **Build one page at a time** — start with the primary/landing page, verify it works, then clone the structure for secondary pages.

4. **Maintain navigation consistency** — every page should share the same sidebar/nav structure with active-state highlighting for the current page.

5. **Preserve prior versions** — when refactoring existing pages to use the new shared system, keep the old versions as `page-name-legacy.html` until the new version is fully verified.

This pattern prevents the common anti-pattern of 8+ pages each containing 1000+ lines of duplicated CSS.
...
### CSS Architecture for Multi-Page Products

When building a product suite with shared UI:

1. **Design Tokens (`design-tokens.css`)** — Define all visual variables in one file:
   ```css
   :root {
   ...
   ```
   Include both dark and light mode variants using `[data-theme="light"]` and `@media (prefers-color-scheme: light)`.

2. **Layout System (`layout.css`)** — Define shared layout scaffolding:
   - Sidebar styles, navigation panels, main content areas
   - Responsive breakpoints (mobile: sidebar becomes bottom nav, nav panel becomes overlay)
   - Z-index scale, scrollbar styling, animation keyframes

3. **Component Library (`components.css`)** — Build reusable components using the tokens:
   - Buttons (primary, secondary, ghost, danger, success variants + sizes)
   - Cards, inputs, tags, badges, status indicators
   - Toast notifications, modals, dropdowns, tooltips, switches
   - Skeleton loaders, empty states, progress bars, code blocks
   - Data tables

4. **Page-Specific Styles** — Each page HTML includes only styles unique to that page, inline in `<style>` or a page-specific CSS file.

This architecture makes the product maintainable and ensures visual consistency across all pages.
...
### Multi-Page Product Suite Verification

When building a product with multiple pages (dashboard, settings, profile, etc.):

1. **Verify shared component system loads correctly** — open the primary page and confirm CSS/JS imports resolve (check Network tab or console for 404s).
2. **Spot-check navigation between pages** — click at least 2-3 nav links to confirm cross-page routing works.
3. **Test global interactions on every page type** — if you built a global search (⌘K), toast system, or theme toggle, exercise it on both the primary page and at least one secondary page.
4. **Check console for CSP or CORS errors** — local file:// URLs often block fetch() or module imports; use a local server (`python -m http.server`) if needed.
5. **Verify responsive behavior at multiple breakpoints** — product suites often have sidebar + main content layouts that collapse differently on tablet vs mobile.
6. **Test data flow** — if pages share a mock API layer, confirm data appears consistently across views (e.g., tasks created on dashboard show up in task hub).
