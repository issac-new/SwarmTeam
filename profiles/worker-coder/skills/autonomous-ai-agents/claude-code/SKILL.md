...
### Large File Edits Time Out
When asking Claude Code to edit large files (>500 lines) via the `Edit` tool in print mode, the operation frequently times out (even with `--max-turns 5` and 120s timeout). This is especially common when the `old_string` spans hundreds of lines.

**Workaround:** For large-file refactoring, use Hermes' native `patch` tool instead of delegating to Claude Code:
1. Use `read_file` with offset/limit to inspect the file structure
2. Use `search_files` to locate specific patterns
3. Use `patch` with `mode='replace'` for the actual edit

**When Claude Code IS the right choice:**
- Small, targeted edits (<50 lines) with precise `old_string`/`new_string`
- File creation (`Write` tool)
- Analysis and reporting (`Read` + natural language questions)
- Multi-file refactoring where Claude can determine the changes autonomously
...
11. **Fall back to native Hermes tools for large file edits** — see `references/print-mode-timeout-handling.md`
