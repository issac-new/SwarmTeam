...
# ── 4. TUI status bar runtime-provider fix ───────────────────────
# The status bar hooks (useCcSwitchModel + useCcSwitchExtra) must read the
# RUNTIME current provider from the proxy's /status (current_provider_id),
# not the DB is_current flag which lags behind failover/circuit-breaker
# trips and freezes the display on stale model/usage/token data.
CCSWITCH_RT_FIX="$HOME/.hermes/profiles/orchestrator/patches/tui-ccswitch-runtime-provider-fix.sh"
if [[ -f "$CCSWITCH_RT_FIX" ]]; then
  _has_rt_fix=$(grep -c 'current_provider_id' "$REPO_DIR/ui-tui/src/components/appLayout.tsx" 2>/dev/null || echo 0)
  if [[ $_has_rt_fix -eq 0 ]]; then
    echo ""
    echo "⚠ TUI status bar runtime-provider fix missing, re-applying..."
    bash "$CCSWITCH_RT_FIX" || true
    # Source patch needs rebuild + TUI restart to take effect
    (cd "$REPO_DIR/ui-tui" && npm run build) || true
  fi
fi
