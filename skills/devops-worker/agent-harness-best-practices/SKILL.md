...
## Capability Seam 三角色模式（借鉴 DeepSeek Harness）

> 来源：deepseek-ai/deepseek-harness `docs/glossary.md` + `docs/capability-seams.md`（2026-08-13）。
> dsh 的核心理念：一个可替换能力（seam）由三个角色完整构成，缺一不可。

### 三角色定义

| 角色 | 定义 | dsh 示例 | Hermes 对应 |
|------|------|---------|-----------|
| **Service Definition** | 声明接口的 Cordis Service（拥有 `ctx.<key>` 和类型词汇） | `dsh-shell`（声明 `ctx.shell`） | toolset 配置声明（config.yaml `toolsets: [terminal, file]`） |
| **Service Provider** | 实现接口的一个或多个后端 | `dsh-bash-local` / `dsh-bash-sandbox` | profile 的 config.yaml（同一个 toolset 不同 provider：local vs Docker） |
| **Consumer** | 使用该服务的调用方，通常是模型可见工具 | `dsh-tool-bash`（模型调用的 bash 工具） | kanban worker（通过 toolset 调用工具） |

### 为什么三角色必须完整

1. **只有 Definition 无 Provider** — 声明了能力但无人实现 → 启动时 fail loud
2. **只有 Provider 无 Definition** — 实现了但接口未声明 → 消费者无法发现
3. **只有 Consumer 无 Provider** — 想用但没人提供 → 运行时 error

### Hermes 场景的 seam 分析

```
能力：Shell 执行
  ├── Definition: toolsets: [terminal]  （config.yaml 声明）
  ├── Provider: terminal backend=local / backend=docker  （config.yaml 配置）
  └── Consumer: kanban worker 的 terminal() 调用  （SOUL.md 操作命令）

能力：编码执行
  ├── Definition: toolsets: [acp]  （config.yaml 声明）
  ├── Provider: acp provider=claude / provider=codex  （ACP 配置）
  └── Consumer: worker-coder 通过 ACP 委托编码  （mandatory-acp.md 规则）

能力：Web 搜索
  ├── Definition: toolsets: [web]  （config.yaml 声明）
  ├── Provider: web_search backend  （Hermes 内置）
  └── Consumer: worker-researcher 的 web_search 调用
```

### seam 健康检查

定期审计每个 toolset 的三角色完整性：

```bash
# 检查每个 profile 的 toolset 声明（Definition）
python3 -c "
import yaml, os
for p in os.listdir(os.path.expanduser('~/.hermes/profiles')):
    cfg = f'{os.path.expanduser(\"~/.hermes/profiles\")}/{p}/config.yaml'
    if os.path.exists(cfg):
        c = yaml.safe_load(open(cfg))
        toolsets = c.get('toolsets', [])
        print(f'{p}: {toolsets}')
"
# 对每个 toolset，确认 Provider 配置存在
# 对每个 toolset，确认至少一个 profile 的 SOUL.md 引用了该能力
```

### 与 microkernel 声明的关系

dsh 的架构宣言：「**每个产品功能都映射到一个文档化扩展点的监听器——微内核声明可检查**」。

Hermes 的等价检查：每个 SOUL.md 中提到的工具能力，都能在 config.yaml toolsets 中找到声明，且至少有一个 provider 实现。不能有"SOUL.md 说了用 X 工具但 config.yaml 没声明"的断裂。
