...
## Pitfall 7: Hermes aux 调用 wire 错配（anthropic_messages → chat_completions 400）

**症状**：Hermes 主对话正常（anthropic wire 200），但 auxiliary 调用（标题生成/压缩/审批/vision）报 400，或 cc-switch 日志里这些请求落在 codex 端点 `/chat/completions`。

**机制**：Hermes aux 客户端按 `api_mode` 选 wire。named custom provider（`custom:cc-switch`）的 `api_mode: anthropic_messages` 若未被尊重（旧版 Hermes 把 named 折叠成 bare custom → URL 探测落空 → chat_completions），或 `anthropic` SDK 不在 venv（静默降级 OpenAI wire，日志有 "anthropic SDK is not installed"），请求就打到 cc-switch codex 端点 → BigModel 400。

**排查**：
```bash
# 1. venv 里 anthropic SDK 必须存在
/Users/YOURNAME/.hermes/hermes-agent/venv/bin/python -c "import anthropic; print(anthropic.__version__)"

# 2. 实测 aux 解析（必须返回 AnthropicAuxiliaryClient）
cd /Users/YOURNAME/.hermes/hermes-agent && ./venv/bin/python -c "
import sys; sys.path.insert(0,'.')
from agent.auxiliary_client import _resolve_auto_route
print(type(_resolve_auto_route(task='title_generation')[0]).__name__)
"

# 3. 端到端 anthropic wire 实测
curl -s -w '\nHTTP %{http_code}\n' http://127.0.0.1:15721/v1/messages \
  -X POST -H 'Content-Type: application/json' -H 'x-api-key: PROXY_MANAGED' -H 'anthropic-version: 2023-06-01' \
  -d '{"model":"glm-5.3-flash","max_tokens":10,"messages":[{"role":"user","content":"hi"}]}'
```

**修复**（三层显式钉死，2026-09-05 已全 50 config 落地）：
1. 每任务 `auxiliary.<task>.api_mode: anthropic_messages`（vision/compression/approval/title_generation/kanban_decomposer 等 11 键，auxiliary_client.py:5461 认此键）
2. `custom_providers[].cc-switch.api_mode: anthropic_messages`（主链路）
3. 顶层 `fallback_providers: [{provider: custom:cc-switch, model: ...}]`（**顶层键**，非 model.fallback_providers；aux 与主对话共用，hermes_cli/fallback_config.py:get_fallback_chain）

**注意**：裸 `requested='custom'` 解析 fallback 到 OpenRouter（chat_completions）——profile 引用 `custom:cc-switch` 但 custom_providers 缺条目时走这条路，必须补条目。
