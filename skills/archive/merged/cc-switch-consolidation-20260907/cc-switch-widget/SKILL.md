...
- **BigModel reset countdown (09-05)** - `nextResetTime` (epoch ms) in each
  TOKENS_LIMIT entry powers the `↻` countdown: status bar & widget both render
  `5h 11% ↻2h13m | 7d 39% ↻3d11h`. `fmtReset()`: <60m→`↻Xm`, <48h→`↻XhYm`, else `↻XdYh`.
  Two sync'd copies of `ccswitch.mjs`: `~/.hermes/tui-widgets/` +
  `~/.hermes/profiles/orchestrator/tui-widgets/` (both must be updated).
