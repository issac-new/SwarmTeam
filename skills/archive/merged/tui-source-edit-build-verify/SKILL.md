...
## Trap: dist Stale Because Patch Re-applied AFTER Build (09-05 实证)

`hermes update` / `skill-lifecycle.py apply` can restore pristine src + build dist
FIRST, then re-apply local patches to src AFTERWARDS — leaving dist built from
UNPATCHED source. Symptom: src has the symbols, running TUI lost the custom
segments, `grep -c <symbol> dist/entry.js` = 0. Diagnosis is the 3-step chain:

```bash
stat -f '%Sm %N' dist/entry.js src/components/<patched>.tsx   # dist older than src = stale
grep -c 'useCcSwitchExtra' dist/entry.js                       # 0 = patch missing from bundle
# fix is just: npm run build  (then restart TUI)
```

A TUI node process keeps the bundle it loaded at startup — rebuilding dist does
NOT affect already-running TUI windows; each must be restarted to pick it up.

## Trap: Tool-Output Redaction Fakes `***` Corruption in .tsx

Reading a .tsx via read_file/grep can DISPLAY `Authorization: auth ***` (secret
redaction applied to tool output), but the on-disk bytes are fine
(`Authorization: auth }`). Before 'repairing' redaction damage, verify with
`od -c` on the file and `npx tsc --noEmit` — if tsc is clean, the file is NOT
corrupted; do not edit. The redaction-on-disk problem the section below
describes happens when a write lands via certain paths; tsc is the arbiter.
