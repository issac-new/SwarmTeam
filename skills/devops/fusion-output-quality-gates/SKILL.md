...
### 门 0：教条式融合 / 贴标签（BLOCKER）

**症状**：外部理论的定义先行，工程例子只当注脚——理论原文复述（"XX论的目标是……核心回答……"），然后一张「理论概念→内部机制」映射表。

**判据**：逐行审映射表——右侧必须携带左侧没有的独立工程判据（阈值/触发条件/命令/排除项）；右侧只是把左侧概念换个名字 = 同义反复 = 0 信息量，删。自测法：把右侧的理论词全部删掉，看还剩什么工程动作；剩不下 = 贴标签，重写。

**纠偏方向**：实践为体、理论为名——先承认集群工程实践早已在做（可观测性工程=信息论的工程化、架构工程=系统论的工程化、测试验证=科学方法论的工程化），理论只负责给实践点名。理论的数学工具不迁移（bit 计量/传递函数/卡尔曼滤波类），只取定性判据。

**规模错配（同族症状）**：千人大厂流程原样套进十人小队——每条流程问「谁消费它、拦住过什么」，无消费方的纯仪式性映射产物整个删除。
...
0. [ ] 映射表每行右侧有独立工程判据（非同义反复）？理论词删掉后右侧仍剩工程动作？
