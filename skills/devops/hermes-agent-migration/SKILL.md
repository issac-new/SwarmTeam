...
real API key examples** (e.g. `sk-f9d…` in `native-mcp/SKILL.md`), so the
...
real_prefixes = [r'sk-f9d', r'sk-ezi', r'gho_pW', r'syt_dGVz', r'p9Jh0dJc', r'desk-12fe8aa6']
...
```bash
grep -rl "sk-f9d\|sk-ezi\|gho_pW\|syt_dGVz\|desk-12fe8aa6" /tmp/hermes-sanitize/ | wc -l
# Must be 0
```
...
with `sk-REDACTED` instead of `sk-f9d…`.
...
(e.g. `"scan for sk-f9d"`). A targeted second pass that replaces these
specific prefixes (`sk-f9d` → `sk-REDACTED`) is needed for a clean
