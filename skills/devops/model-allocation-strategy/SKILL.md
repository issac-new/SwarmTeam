...
- **Refusal handling is the model service's responsibility**: if a model
  refuses legitimate work, switch that profile to a different model via
  `hermes config set model.default <alt> --profile <name>`. Do not add
  local prompt-level authorization/refusal-mitigation scaffolding.
