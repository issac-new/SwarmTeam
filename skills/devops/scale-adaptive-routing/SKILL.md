...
### 1.5 选型/问答类前置裁决门（2026-09-04 用户裁决）

> 判例：「底层存储共用 minio 或 rustfs」一个选型问题实际耗时 1h+（被纪律定性为重型变更：
> 取证→选型→compose 改造→容器重建→三链路实证→同步设计文档），用户追问原因后确立此门。

**规则（回答前过一遍）**：
1. 选型/问答类请求（「用 X 还是 Y」「现在是什么版本」）→ **先回结论 + 实施预估工时**，
   等用户裁决后再动手；不默认连做带验
2. 动手前把长等待步骤显式报 ETA（集群扫描周期/快照间隔/容器健康检查/大镜像层下载），
   让用户知道在等什么而非认为卡死
3. 拉镜像前先 `docker images` 查本地，已有即跳过
4. 镜像源**慢失败**（爬几分钟才报「繁忙」）不原路重试，直接换通道

**判定顺序**：先过本门（是不是纯问答/选型？）→ 再过 §1 blast-radius 门定轨道。
