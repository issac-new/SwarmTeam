...
This copies the plugin directory in one shot. No need to re-patch each profile individually.

> ⚠️ **After running Step 2, ALWAYS run the Codex compatibility fix** (see Step 2b below).
> The copied `__init__.py` may be an older version without Codex ACP protocol fixes
> (`_do_permission` outcome format + `_resolve_provider` codex sandbox args). The fix
> script at `~/.hermes/patches/apply-acp-client-codex-fix.sh` handles this automatically.

### Step 2b: Re-apply Codex compatibility fix (CRITICAL)

```bash
bash ~/.hermes/patches/apply-acp-client-codex-fix.sh
```

This copies the golden-copy `__init__.py` (with Codex ACP protocol fixes) to all
13 active profiles and kills any running codex-acp processes. Safe to re-run.
