...
## rules.md Authoring for k12edu team (8.10 + 8.13 mandatory components)

Every k12edu `<profile>/rules.md` MUST contain 4 components, mechanically
grep-verifiable: (1) 8.10 范式转移 (日常微互动/关系导向/传递的C/过程性反馈),
(2) 8.13 成长型思维 (成长型思维/你不笨/接纳情绪/需要练习 — 孩子说「我太笨了」禁止「你不笨」),
(3) 情境感知 (当前行程状态 — 推送前必读 child-profile.md 顶部行程块),
(4) 学科验证标准 (验证标准 — 量化验证表).
Profile-specific: orchestrator=调度验证门禁; character=四个C+成长型话术模板;
stem=结构化逻辑游戏; chinese=故事化+具象叙事文物(杜虎符); language=多通道;
arts=假想游戏+道具脚手架梯度; physical=感统+原始反射整合.

Pitfalls: (a) execute_code 50-call cap truncates per-file python loops → use
single shell `for` loop or `grep -E`. (b) read_file flags these UTF-8 files as
binary → use terminal cat/sed/grep. (c) k12-character rules.md may pre-exist
richer team-authored → PATCH not overwrite.
