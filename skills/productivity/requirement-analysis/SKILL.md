...
description: "需求澄清、补充、验证。输出结构化需求规格文档（PRD/用户故事/验收标准）。适用于 orchestrator triage 或复杂任务需求阶段。"
version: 1.0.0
...
metadata:
  hermes:
    ...
    source_profile: requirement-analyst
---

# Requirement Analysis

> 本 skill 从 `requirement-analyst` profile 降级而来。原 profile 已归档（`.archived`），其核心方法论沉淀为本 skill 供按需加载。
...
3. **需求澄清** — 分析需求完整性，识别缺失信息
4. **需求验证** — 确认业务背景和约束条件
5. **输出文档** — 标准化需求规格文档（Markdown）
6. **完成** — `kanban_complete()` 提交文档路径和关键信息
