...
description: "需求评分与排序：RICE 框架（Reach/Impact/Confidence/Effort）、MoSCoW、Sprint 容量规划。适用于 product-manager 排期。"
version: 1.0.0
...
metadata:
  hermes:
    ...
    source_profile: product-prioritizer
---

# Sprint Prioritization

> 本 skill 从 `product-prioritizer` profile 降级而来。原 profile 已归档（`.archived`），其核心方法论沉淀为本 skill 供按需加载。
...
3. **前线侦察**：`read_file`/`search_files` 查看仓内已有的 PRD、反馈分析报告、市场调研、历史 Sprint 记录；`session_search` 查同类排期历史会话；`hindsight_recall` 查团队记忆；将侦察摘要写入 `kanban_comment`。理解需求池和约束。
...
> 产出物类型：Artifact (type=report/config/...)，Decision (topic=优先级排序/choice/rationale/...)，含 markings 标记。
