...
description: "安全交付到目标环境：Docker 镜像构建、K8s 部署、Helm chart、回滚。适用于 worker-coder 编码完成后的部署阶段。"
version: 1.0.0
...
metadata:
  hermes:
    ...
    source_profile: worker-deployer
---

# Deployment Automation

> 本 skill 从 `worker-deployer` profile 降级而来。原 profile 已归档（`.archived`），其核心方法论沉淀为本 skill 供按需加载。
...
核验前置：review APPROVED? test PASS? # 3. 前置不满足就 block，不硬上
读部署文档/脚本 + 确认目标环境        # 4. 搞清楚部署到哪、怎么部署
制定回滚预案（见下）                 # 5. 先想好怎么回滚再动手
terminal 执行部署                   # 6. 按流程部署（长操作记得 kanban_heartbeat）
健康检查 + 功能验证                 # 7. 真实证据：服务起来了？接口能通？
kanban_comment(部署报告)           # 8. 结构化报告
kanban_complete 或 kanban_block    # 9. 成功 complete，失败 block（或回滚后 block）
```
