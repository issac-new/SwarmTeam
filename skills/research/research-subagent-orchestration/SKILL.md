...
description: "Use when dispatching 2+ parallel research subagents."
---

# Research Subagent Orchestration

Patterns for dispatching, monitoring, recovering, and merging parallel
research subagents for multi-dimensional OSINT/research tasks. Complements
`deep-research-workflow` (overall research workflow) with the specific
subagent lifecycle patterns discovered in the phlexing OSINT session
(3 parallel subagents producing a 40KB unified intelligence report) and the
2026-09-04 AI-status-report session (3-way arXiv/WeChat/industry split).
...
Give each child: today's date, its own absolute output directory (create it
first), a hard constraint that all output must be written to disk there, its
exclusive scope ("you do NOT touch the other dimensions"), known channel
lessons/pitfalls it would otherwise rediscover, and a required JSON summary
schema (status / output_files / counts / key_findings) so the merge step can
...
contains all tool call results. `tail` it to monitor children mid-run and
detect drift early; mine it with regex for structured data (repos, papers,
DNS records) when a child dies.

### Pattern 5: Parent collects its own baseline while children run

While background children work, the orchestrator collects independent,
mechanically verifiable baseline data itself (e.g. arXiv totalResults counts,
official-site indexes) and drafts the report skeleton with placeholder
sections. Three payoffs (2026-09-04 AI-status-report session):

1. The final report keeps hard data even if a child fails or times out.
2. Cross-checking child claims against parent-collected baselines catches
   fabrication cheaply (pairs with worker-completion-independence-verification).
3. Sunk setup cost is protected: if the parent's own collection partially
   fails (e.g. arXiv 429 mid-series), DELETE the failed placeholder files
   immediately and annotate the gap in the draft's appendix — a stray
   `{"total": -1}` JSON left in the workspace poisons the evidence chain
   and a later reader may cite it as real data.

A pre-written skeleton also makes the merge step mechanical: each child
summary drops into a marked placeholder instead of a blank page.
...
- **Do NOT leave failed placeholder files in the workspace**: invalid
  sentinel values (-1, null) on disk get cited as real data by later readers.
  Delete + annotate in the report appendix instead.
