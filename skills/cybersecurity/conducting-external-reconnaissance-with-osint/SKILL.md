...
**Read `references/commands.md` first** — it contains the Threatswarm-style phased
workflow with copy-paste-ready command templates (subfinder/amass/crt.sh, httpx,
nmap full-port, nuclei, feroxbuster, theHarvester, cloud_enum/Pacu/ScoutSuite,
apktool/frida/objection as applicable). Follow the phases in order; each phase
includes tool install assumptions and output file naming conventions.
